...
class YITH_WCMV_Addons {
    ...
    public function get_plugin_landing_uri( $plugin_name, $context = 'uri' ){
        $plugin_link = '';
        if ( 'display' == $context ) {
            $coming_soon = ! empty( $this->plugins[$plugin_name]['coming_soon'] ) && $this->plugins[$plugin_name]['coming_soon'] ? sprintf( '<span class="yith-wcmv-add-ons coming-soon">%s</span>', __( 'Coming Soon', 'yith_wc_product_vendors' ) ) : '';
            $is_new      = ! empty( $this->plugins[$plugin_name]['is_new'] )      && $this->plugins[$plugin_name]['is_new'] ? sprintf( '<span class="yith-wcmv-add-ons is-new">%s</span>', __( 'New', 'yith_wc_product_vendors' ) ) : '';
            $plugin_link = $this->has_plugin( $plugin_name ) ? sprintf( '%s', $is_new )  : sprintf( '<span class="yith-wcmv-required-plugin">(%s <a href="%s" target="_blank">%s</a> %s - %s %s %s).</span>%s',
                ...
                _x( 'or greater', 'means: min version xxx or greater', 'yith_wc_product_vendors' ),
