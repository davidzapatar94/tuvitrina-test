...
if ( ! class_exists( 'YITH_Vendors_Admin_Premium' ) ) {
    ...
    class YITH_Vendors_Admin_Premium extends YITH_Vendors_Admin {
        ...
        public function quick_edit_render( $column_name, $post_type ) {
            ...
            if ( $enabled && $post_type == 'product' && $column_name == $vendor_col_name ) {
        ...
        public function get_columns( $columns ) {
            ...
            $to_add = array(
                ...
                'products'          => __( 'Items', 'yith_wc_product_vendors' ),
        ...
        public function disabled_manage_other_vendors_posts() {
            ...
            if ( $is_seller ) {
                ...
                if (
                    ...
                    wp_die( sprintf( __( 'You do not have permission to edit this order. %1$sClick here to view your orders%2$s.', 'yith_wc_product_vendors' ), '<a href="' . esc_url( 'edit.php?post_type=shop_order' ) . '">', '</a>' ) );
                }
            }
        }
        ...
        public function vendor_recent_comments_widget() {
            ...
            if ( $comments ) {
                ...
                echo '<h4>' . __( 'Comments', 'yith_wc_product_vendors' ) . '</h4>';
        ...
        public function add_sold_by_to_order( $item_id, $item, $_product ) {
            ...
            if ( $vendor_by_product && $vendor_by_product->is_valid() && $current->id != $vendor_by_product->id ) {
                ...
                echo ' (<small>' . apply_filters( 'yith_wcmv_sold_by_string_admin', _x( 'Sold by', 'Order details: Product sold by', 'yith_wc_product_vendors' ) ) . ': ' . '<a href="' . $vendor_uri . '" target="_blank">' . $vendor_by_product->name . '</a></small>)';
            }
        }
                    ...
                    <div class="notice notice-warning">
                        <?php if( ! empty( $no_owner_shop ) ) : ?>
                            <p>
                                ...
                                if( ! $is_vendor_taxonomy_page ) {
                                    printf( ' <a href="%s">%s</a>', esc_url( add_query_arg( array( 'post_type' => 'product', 'taxonomy' => YITH_Vendors()->get_taxonomy_name(), 'orderby' => 'owner' ), admin_url( 'edit-tags.php' ) ) ),  __( 'Go to Vendor page to fix it.', 'yith_wc_product_vendors' ) );
                                }
                                ?>
                            </p>
                        <?php endif; ?>
                        ...
                        <?php if( ! empty( $no_owner_vat ) ) : ?>
                            <p>
                                ...
                                if( ! $is_vendor_taxonomy_page ) {
                                    printf( ' <a href="%s">%s</a>', esc_url( add_query_arg( array( 'post_type' => 'product', 'taxonomy' => YITH_Vendors()->get_taxonomy_name() ), admin_url( 'edit-tags.php' ) ) ),  __( 'Go to Vendor page to fix it.', 'yith_wc_product_vendors' ) );
                                }
                                ?>
                            </p>
                        <?php endif; ?>
                    ...
                    <div class="notice notice-warning">
                        <p>
                            ...
                            if( ! $is_vendor_details_page ) {
                                printf( ' <a href="%s">%s</a>', esc_url( add_query_arg( array( 'page' => 'yith_vendor_settings', 'tab' => 'vendor-settings' ), admin_url( 'admin.php' ) ) ),  __( 'Go to Vendor details page to fix it.', 'yith_wc_product_vendors' ) );
                            }
                            ?>
                        </p>
                    </div>
        ...
        public function add_wp_editor_to_vendor_tax() {
            ...
            if ( 'yes' == get_option( 'yith_wpv_vendors_option_editor_management', 'no' ) && $this->is_vendor_tax_page() && ! empty( $_REQUEST['tag_ID'] ) ) {
                ...
                ?>
                <tr class="form-field">
                    <th scope="row" valign="top"><label for="description"><?php _ex('Description', 'Taxonomy Description', 'yith_wc_product_vendors'); ?></label></th>
                    <td>
                        <?php $this->add_wp_editor( $vendor->description, array( 'textarea_name' => 'description', 'textarea_id' => 'textarea#description'), true ); ?>
                        <br />
                        <span class="description"><?php _e( 'The description is not prominent by default; however, some themes may show it.', 'yith_wc_product_vendors' ); ?></span>
                    </td>
                </tr>
                <?php
            }
